...
def aux_loss(router_logits: torch.Tensor, num_experts: int, top_k: int):
    ...
    # Select the top-k experts for each token in the sequence
    _, selected_experts = torch.topk(routing_weights, top_k, dim=-1)
    
    # Create a one-hot tensor for selected experts
    expert_mask = F.one_hot(selected_experts, num_classes=num_experts).sum(dim=1)
    
    # Calculate the fraction of tokens per expert
    num_tokens = expert_mask.sum()
    fraction_of_tokens = expert_mask.float() / num_tokens
